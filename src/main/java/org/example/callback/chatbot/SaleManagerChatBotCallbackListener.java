...
import org.example.service.SaleManagerMessagesService;
...
@Slf4j
@Component
public class SaleManagerChatBotCallbackListener implements OpenDingTalkCallbackListener<ChatbotMessage, JSONObject>, InitializingBean {
    private BlockingQueue<ChatbotMessage> messageQueue = new ArrayBlockingQueue<>(1000);

    private SaleManagerMessagesService saleManagerMessagesService;

    @Autowired
    public SaleManagerChatBotCallbackListener(SaleManagerMessagesService saleManagerMessagesService) {
        this.saleManagerMessagesService = saleManagerMessagesService;
    }
    ...
    private void executeInfo(ChatbotMessage chatbotMessage) {
        try {
            saleManagerMessagesService.sendPrivateMessage(chatbotMessage);
        }catch (Exception e){
            log.warn("执行业务逻辑错误",e);
        }
    }
